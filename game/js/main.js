var isGameOver = false; // Add this flag
...
function endgame(){
isGameOver = true; // Set the flag to true when the game ends
$("#changer").css("display", "none");
$("#endgamer").css("display", "block");
dieplayer();

// Disable the jump arrow
$("#jump-arrow").off("click"); // Remove the click event listener
$("#jump-arrow").css("pointer-events", "none"); // Disable pointer events
$("#jump-arrow").css("opacity", "0.5"); // Optionally, make it look disabled
} 

function restartGame() {
  isGameOver = false; // Reset the game over flag
  $("#jump-arrow").css("pointer-events", "auto"); // Re-enable pointer events
  $("#jump-arrow").css("opacity", "1"); // Restore opacity
  $("#jump-arrow").on("click", function() {
    player(); // Reattach the jump functionality
  });
